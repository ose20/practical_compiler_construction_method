...
let rec ast_stmt ast = 
    ...
    | CallProc (s, l) -> 
        sprintf "CallProc(\"%s\",[%s])" s 
            (List.fold_left (fun str exp -> (semi str) ^ (ast_exp exp)) "" l) 
    | Block (dl, sl) -> 
        sprintf "Block([%s],[%s])" 
        (List.fold_left (fun str dec -> (semi str) ^ (ast_dec dec)) "" dl) 
        (List.fold_left (fun str stmt -> (semi str) ^ (ast_stmt stmt)) "" sl) 
    | Assign (v, e) -> 
...
and ast_var ast = 
    ...
    | Var s -> 
        sprintf "Var \"%s\"" s 
    | IndexedVar (v, e) -> 
        sprintf "IndexedVar (%s,%s)" (ast_var v) (ast_exp e)
and ast_dec ast = 
    match ast with 
    | FuncDec (s, l, t, b) ->
...
and ast_typ ast = match ast with 
    | NameTyp s -> 
        sprintf "NameTyp \"%s\"" s 
    | ArrayTyp (size,t) -> 
        sprintf "ArrayTyp (%d,%s)" size (ast_typ t) 
    | IntTyp -> 
        "IntTyp" 
    | VoidTyp -> 
        "VoidTyp"

let main () = (* The open of a file *) 
    let cin = 
        if Array.length Sys.argv > 1 
        then open_in Sys.argv.(1) 
        else stdin 
    in 
    let lexbuf = Lexing.from_channel cin in (* The start of the entire program *)
